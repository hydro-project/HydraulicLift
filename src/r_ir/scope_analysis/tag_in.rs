...
impl From<RStmtWhile> for TS<RStmtWhile<ScopeDef>> {
    fn from(RStmtWhile { box cond, box body }: RStmtWhile) -> Self {
        TS::from(cond).and_then(|cond| TS::from(body).map(|body| RStmtWhile::new(cond, body)))
    }
}
